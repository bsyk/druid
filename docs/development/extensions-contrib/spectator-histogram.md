...
adds 48 bytes per row. This is a 8x reduction in additional storage size.
...
`quantilesDoublesSketch` adding 255 bytes per row. This is a 9.4x reduction in additional storage size.
...
It was built, primariily, to work with [Atlas](https://netflix.github.io/atlas-docs/).
...
datastore to benefit from high-dimensionality and high-cardinality data.
...
## Functionality
The SpecatorHistogram aggregator is capable of generating histograms from raw numeric
values as well as aggregating/combining pre-aggregated histograms generated using
the SpecatorHistogram aggregator itself.
...
percentiles from histograms generated by the SpecatorHistogram aggregator.
Again, these postAggregators can be used to compute percentiles from raw numeric
values via the SpecatorHistogram aggregator or from pre-aggregated histograms.

> If you're only using the aggregator to compute percentiles from raw numeric values,
then you can use the built-in data-sketch aggregator instead since the performance
...
aggregator to reteive the population of the histogram. This is effectively the count
...
To use SpecatorHistogram, make sure you [include](../../configuration/extensions.md#loading-extensions) the extension in your config file:
...
a collection of timer values. It is recommened to normalize timer values to nanoseconds
...
### Percentile (singular)
This returns a single percentile calcuation based on the distribution of the values in the aggregated histogram.
